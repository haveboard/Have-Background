...
<div id="background_options">
	...
	<fieldset class="radio"> 
		...
		<?
			...
			?>
			<label><input type="radio" name="background_repeat" value="repeat-x" checked="checked" />  repeat-x</label> 
			<?
			}else{
			?>
			<label><input type="radio" name="background_repeat" value="repeat-x" />  repeat-x</label> 
			<?		
			}
			if($background_repeat == 'repeat-y') {
			?>
			<label><input type="radio" name="background_repeat" value="repeat-y" checked="checked" />  repeat-y</label> 
			<?
			}else{
			?>
			<label><input type="radio" name="background_repeat" value="repeat-y" />  repeat-y</label> 
			<?		
			}
...
<?php
	...
	//add_action("admin_init", "add_background");
...
?>
<link rel="stylesheet" media="screen" type="text/css" href="<?php dirname( plugin_basename( __FILE__ ) ); ?>/wp-content/plugins/colorpicker/css/colorpicker.css" />
<script type="text/javascript" src="<?php dirname( plugin_basename( __FILE__ ) ); ?>/wp-content/plugins/have_background/colorpicker/js/colorpicker.js"></script>


    <script type="text/javascript" src="<?php dirname( plugin_basename( __FILE__ ) ); ?>/wp-content/plugins/have_background/colorpicker/js/eye.js"></script>
    <script type="text/javascript" src="<?php dirname( plugin_basename( __FILE__ ) ); ?>/wp-content/plugins/have_background/colorpicker/js/utils.js"></script>
    <script type="text/javascript" src="<?php dirname( plugin_basename( __FILE__ ) ); ?>/wp-content/plugins/have_background/colorpicker/js/layout.js"></script>
<?
...
if((isset($bg_id) && $bg_id != "")){
	...
	$index_background = new WP_Query("post_type=background&post_status=publish&posts_per_page=1&orderby=rand");


if ($index_background->have_posts()) :
	while ($index_background->have_posts()) : $index_background->the_post(); 
	
$background_url= get_first_image();


		$bg_color = get_post_meta($post->ID, "background_color", true);
echo "<!--".$bg_color."-->";
		$bg_repeat = get_post_meta($post->ID, "background_repeat", true);
		if($bg_repeat != ""){
			$bg_repeat = "".$bg_repeat;
		}else{
			$bg_repeat = "repeat";
		...
		$bg_position_horizontal = get_post_meta($post->ID, "background_position_horizontal", true);
		if($bg_position_horizontal != ""){
			if($bg_position_horizontal == 0){
				$bg_position_horizontal = "".$bg_position_horizontal;
			}else{
		...
		$bg_position_vertical = get_post_meta($post->ID, "background_position_vertical", true);
		if($bg_position_vertical != ""){
			if($bg_position_vertical == 0){
				$bg_position_vertical = "".$bg_position_vertical;
			}else{
		...
		$bg_attach = get_post_meta($post->ID, "background_attachment", true);
		if($bg_attach != ""){
			$bg_attach = "".$bg_attach;
		}else{
			$bg_attach = "fixed";
